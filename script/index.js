        ...
        var Entry = /** @class */ (function () {
            function Entry(mediaElement, gainOnly) {
                ...
                if (gainOnly) {
                    ...
                    this.spliter = Analyser.audioContext.createChannelSplitter(2);
                    this.analyserNodes =
                        {
                    ...
                    this.gainNode = Analyser.audioContext.createGain();
                    this.mediaElementAudioSourceNode = Analyser.audioContext.createMediaElementSource(mediaElement);
                    this.mediaElementAudioSourceNode.connect(this.spliter);
                    this.spliter.connect(this.analyserNodes.left, 0);
                    this.spliter.connect(this.analyserNodes.right, 1);
                    this.mediaElementAudioSourceNode.connect(this.gainNode);
                    this.gainNode.connect(Analyser.audioContext.destination);
            ...
            Entry.prototype.destroy = function () {
                ...
                (_e = this.spliter) === null || _e === void 0 ? void 0 : _e.disconnect();
            ...
            Entry.prototype.getChannelCount = function () {
                return this.mediaElementAudioSourceNode.channelCount;
            };
            Entry.prototype.mixToMono = function (left, right, mono) {
                var length = Math.min(left.length, right.length, mono.length);
            ...
            Entry.prototype.getByteFrequencyData = function (channel) {
                ...
                if ("mono" === channel) {
                    if (this.analyserNodes && !this.isValidFrequencyData.mono) {
                        ...
                        var left = this.getByteFrequencyData("left");
                        var right = this.getByteFrequencyData("right");
                        if (left && right) {
                            this.mixToMono(left, right, this.frequencyDataArray.mono);
                ...
                if ("mono" === channel) {
                    if (this.analyserNodes && !this.isValidTimeDomainData.mono) {
                        if (!this.timeDomainDataArray.mono) {
                            this.timeDomainDataArray.mono = new Uint8Array(Analyser.fftSize);
                        }
                        var left = this.getByteTimeDomainData("left");
                        var right = this.getByteTimeDomainData("right");
                        if (left && right) {
                            this.mixToMono(left, right, this.timeDomainDataArray.mono);
...
define("script/features/visualizer", ["require", "exports", "script/library/index", "script/ui", "resource/config"], function (require, exports, _library_4, ui_3, config_json_4) {
    ...
    (function (Visualizer) {
        ...
        Visualizer.getAngle = function (channel, startAngle, rate) {
            switch (channel) {
                case "left":
        ...
        Visualizer.drawArcFrequency = function (context, channel, rect, scale, analyser) {
            ...
            if (context && frequencyDataArray) {
                ...
                for (var i = 0; i < maxIndex; i++) {
                    var hue = (i / maxIndex) * config_json_4.default.visualizer.maxHue;
                    var angle = Visualizer.getAngle(channel, startAngle, i / maxIndex);
                    var value = frequencyDataArray[i] / 255.0;
        ...
        Visualizer.drawArcWaveform = function (context, channel, rect, scale, analyser) {
            ...
            if (context && timeDomainDataArray) {
                ...
                context.lineTo(Visualizer.getPointAtAngle(center, Visualizer.getAngle(channel, startAngle, 1.0), radius));
                context.stroke();
            }
        };
